 ...
 * 
 * Updated by: Samarth Sharma, Masoumeh Mirzaeepour Gelvarzkhah, Abdullah Zeki Ilgun, Dishaben Patel
 *
 */
...
public class PersonResource {
	...
	@GET
	@RolesAllowed({ADMIN_ROLE})
	@Path("/associate/{personId}/{phoneId}/{addressId}")
	public Response associatePhone(@PathParam("personId")Integer personId, @PathParam("phoneId")Integer phoneId, @PathParam("addressId")int addressId) {
		if(phoneId == null || personId == null) {
			return Response.status(Status.BAD_REQUEST).build();
		}
		try {
		return service.associatePhoneAndAddress(personId, phoneId, addressId);
		}catch(Exception e) {
			e.printStackTrace();
		}
		return null;
